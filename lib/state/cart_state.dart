...
class CartState with ChangeNotifier {
  ...
  Future<void> getCartDatas() async {
    String url = 'http://10.0.3.2:8000/api/cart/';
    var token = storage.getItem('token');
    try {
  ...
  Future<void> getoldOrders() async {
    String url = 'http://10.0.3.2:8000/api/order/';
    var token = storage.getItem('token');
    try {
      http.Response response = await http.get(Uri.parse(url), headers: {
        "Authorization": "token $token",
  ...
  Future<void> addtoCart(int id) async {
    String url = 'http://10.0.3.2:8000/api/addtocart/';
    var token = storage.getItem('token');
    try {
  ...
  Future<void> deletecartproduct(int id) async {
    String url = 'http://10.0.3.2:8000/api/delatecartprod/';
    var token = storage.getItem('token');
    try {
      http.Response response = await http.post(Uri.parse(url),
          body: json.encode({
  ...
  Future<bool> deletecart(int id) async {
    String url = 'http://10.0.3.2:8000/api/deletecart/';
    var token = storage.getItem('token');
    try {
      http.Response response = await http.post(Uri.parse(url),
          body: json.encode({
  ...
  Future<bool> ordercart(
    ...
    String url = 'http://10.0.3.2:8000/api/ordernow/';
    var token = storage.getItem('token');
    try {
      http.Response response = await http.post(Uri.parse(url),
          body: json.encode({
